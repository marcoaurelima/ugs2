...
/// OBS.: a cada mudan�a que for ser efetuada, deve-se criar uma nova instancia dessa classe!!!! /////////
...
UGSMenuMainDetails::UGSMenuMainDetails()
{
    ...
    /// este sprite mInstruments[15] representar� provisoriamente um instrumento inexistente//sera transparente
    for(unsigned i=0;i<mInstruments.size();i++){
        mInstruments[i].setPosition(0, -100); /// posi��o deixar� fora de vista usando posicao fora da tela
    }

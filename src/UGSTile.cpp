...
{
    /// Defini provisoriamente que a dura��o ser� em SEGUNDOS ///
    ...
    if(mDuration > 0){
        ...
        /// Proporcao do tile se for com dura�ao 0;
    ...
    /// convencional dava erro de aloca��o ou algo do tipo.
    /// Acho que isso se deve a aloca��o dinamica de tiles que eu estou fazendo em UGSGameMajor.
    /// Curiosamente deu bug quando tentei destruir o ponteiro no destrutor. Ent�o deixei o ponteiro solto.
...
void UGSTile::draw(sf::RenderWindow& window, sf::Transform transform, int& errorControlRock, sf::Music& music){
    ...
    ///� partir de 570px o rastro apaga
    if(mTile.getPosition().y > 570 && !mPlayerError){
        if(!mTileFinalized){ /// So ir� fazer som de erro quando o tile estiver finalizado, com o rastro totalmente gasto
            mSoundError->play();
            errorControlRock--;
            music.setVolume(10);
        }
...
void UGSTile::wast(){
    ...
    ///Como o numero de velocidades jogaveis � limitado (no momento 8 op�oes),
    ///resolvi buscar constantes de calculo de propor��o do rastro da nota ao

...
void UGSGameMajor::gameButtonsControl(){
    ...
    /// se nao, ele sera falso e emitir� o som de erro;
    bool tileHitted[5] = {false,false,false,false,false};

    /// Essa variavel controla o pressionamento de botao para n�o ficar
    ...
    if(key_pressed_winapi(mKeyboardConfig[0]) ){
        ...
        for(unsigned i=0;i<mTile.size();i++){
            if(mTile[i].getPosition().y > 490 && mTile[i].getPosition().y < 580
                ...
                /// Enquanto o som de erro n�o parar, ficara bloqueado o hit do tile.
...
void UGSGameMajor::draw(sf::RenderWindow& window){
   /// aqqui ser� detectado a diferenca de pontua��o frame a frame para o calculo do ROCK.
   /// se a diferen�a for pra mais, ele incrementa, se for pra menos, ele decrementa;
   ...
   if(mConsecutiveNotesNow > rockControl) { mRock++; } else if
    ...
    /// aqui ser� calculado as notas consecutivas e tudo relacionado com o vector de bool que guarda essas informa��oes (mSequenceBoolTiles)
...
//mMusic
    ...
    /// DEFINICAO DO MULTIPLICADOR: DE QUANTAS NOTAS CONSECUTIVAS PRECISO PRA AUMENTYAR +1 (POR ENQUANTO SER� 10 COM LIMITE DE 5X)
    ...
    /// Gera��o dos Tiles em tempo real.
    ...
    /// guarda as informa��es dos tiles. Se isso acontecesse, ele geraria mais pe�a e seriam aleatorias;
    ...
    for(unsigned i=0;i<mTile.size();i++){
        /// Destrui��o de tiles em tempo real.
